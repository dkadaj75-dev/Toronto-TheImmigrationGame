...
## R4 — SHIPPED (2026-07-17)

As-built: PendingMoveTracker (game/rental.ts) on the gameHourNow clock — start refuses concurrent moves, cancel applies NOTHING, takeCompleted is the only switch path; completion deferred while at-work/buy-mode/repo/game-over. Runtime switch reuses the extracted applyFreshData hot-reload closure: KEEP stats/funds/quests/visa/job/credit/bills/camera; DROP accidents, buy-mode overlay (furniture left behind, no refund — flagged fork), carried food/waste, garbage fill, ON/OFF state (per-system comments in completePendingMove). homeMap: quests.vars + read-modify-write PUT of data/simstate.json (only sanctioned write); boot resolveHomeMapId(simstate, tuning). Finance recomputes from live data.map next bills tick (verified at source). Kijiji card: enabled Rent, pending countdown + Cancel. NOTE for designer: once homeMap is set, tuning.map.active no longer picks the played map until homeMap changes (documented 6.1 consequence). Suites: pendingmove + rental/phone/phone-hud/nav/quests/bills/locomotion.

**R4 amendment (2026-07-17, B10-22):** designer superseded decision 6.1 — homeMap does NOT persist (runtime PUT removed, in-memory only until the save system; boot still reads resolveHomeMapId for forward-compat).

## D2 — SHIPPED (2026-07-17)

As-built: AssetDef.door.paneNode (node name inside the single mesh GLB; wins if both set) / door.paneMesh (separate GLB combined with the frame in one group, normalize+meshFit applied to the COMBINED bounds). Pure resolvePaneConfig/paneHingeLocal/paneLocalOffsetFromHinge in doors.ts — hinge on the pane's own swing-side edge (sign of hingeOffset[0] picks min/max X), thickness-center Z; only paneGroup carries the swing angle, frame static; no pane configured = whole asset pivots byte-identically. Fallbacks: stand-in box until load, paneNode-missing/paneMesh-fail warn once + degrade. Asset Editor Door card: pane node name + pane mesh GLB inputs; preview shows the combined pair and lists GLB node names in #preview-msg. Also: wallaperture live-data smoke rewritten self-deriving (designer now legitimately ships on-wall doors; hardcoded gap-only counts removed).

## D3 — SHIPPED (2026-07-17)

As-built: walls[].kind 'solid'(absent)/'curtainWall' — glazing renders the SAME aperture/segment output as solid walls (no special casing; balcony door = normal D1/D2 door) as translucent panes (windowPane precedent), kind wins over texture (glazing untextured), black top + wall-cut behavior preserved; vertical mullions from pure mullionPositions(len, spacing, apertures) skipping doorway spans (tuning.facade.mullionSpacingMeters, default 1.2). floors[].outdoor: walkable as normal, EXCLUDED from rent tile counting (bills.ts countFloorTiles, 6.3). Map Editor: wall kind dropdown (texture pickers hidden on curtainWall), floor outdoor checkbox.

## D4 — SHIPPED (2026-07-17)

As-built: sparse MapData.exterior {skyColor, groundColor, backdrop (GLB=distant mesh / image=inside-textured billboard ring), backdropDistance (default 60), fog{color,near,far, guarded}} resolved by pure game/exterior.ts resolveExterior (test/exterior.test.ts, 34 checks). world.ts buildExteriorInto builds ground+backdrop INTO the world group (rebuild-safe, keep-stand-in on load failure, raycast disabled, no wallCutVisual, never nav-relevant); applyExteriorScene sets sky background + THREE.Fog, called at boot AND inside applyFreshData so hot-reload + R4 map switch swap the exterior. applyDayNight lerps custom sky + ground tint by daylightFactor (O(1)). Map Editor "Exterior (3D environment)" card: color pickers + clearable text, normalized backdrop path, distance, sparse fog block.

## D5a — SHIPPED (2026-07-17)

As-built: collapsible full-width 3D preview panel in tools/map.html — module script (window.MapPreview) renders the IN-MEMORY map via the game's own buildWorld + buildDoors + applyWallCutView + applyExteriorScene + makeLights (nothing reimplemented, zero game/*.ts changes); debounced 180ms rebuild hooked into redraw(); disposal mirrors main.ts (sharedResource-aware); camera reframes only on map-id change. Wall-cut checkbox mirrors in-game view. One-way selection sync: pure previewHighlightBox(doc, sel, assets) (inline, 13 jsdom checks) drives a depth-off wireframe; object boxes refine from real mesh bounds when loaded. 3D clicks do nothing (D5b).
