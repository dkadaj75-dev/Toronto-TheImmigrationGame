...
const saved=JSON.parse(rawPuts['finance.json']),savedTuning=JSON.parse(rawPuts['tuning.json']);check(saved.rent.base===150&&saved.negativeGraceDays===5,'finance PUT contains edited values');
check(savedTuning.credit.repoDelta===-125,'tuning PUT contains edited credit values');
check(rawPuts['finance.json']===JSON.stringify(saved,null,2)&&rawPuts['tuning.json']===JSON.stringify(savedTuning,null,2),'saves use exact pretty whole-file JSON');
